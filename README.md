...
Generally a BTC Private Key (the thing that provides an access to funds stored under the specific BTC address) is an integer value of a 256 bits length (256 of {0 or 1}) or 64 HEXadecimal (16 base) values. The BTC address taken from the explorer above[2] actually obtained form an appropriate (and unknown) BTC private keys through a series of HASHING procedures that takes computing time. Generally, to find a Private Key for the specific BTC address cracker will need to try all 2^256 ({0/1} x {0/1} x ... {0/1}) possible Private Keys, generating a BTC address for each and comparing with an interesting address.
