% Dummy MATLAB script - composes a "task" for "aimed" search of the
...
%% Clear all: command window, figures and workspace
clc;
close all;
clear all;
...
data = readtable('Pzl32_unspentList.csv');
bits = data.Var1;
addresses = data.Var2;
...
for i = 1:length(bits)
    for j = 1:length(alpha_to_seek)
        ...
        left_edge = floor( vpa(2^(bits(i)-1), vpa_acc) * (1+alpha_to_seek(j)) ) + floor(-MAX_Keys_interval/2);
        right_edge = ceil( vpa(2^(bits(i)-1), vpa_acc) * (1+alpha_to_seek(j)) ) + ceil( MAX_Keys_interval/2);
        ...
        if (left_edge - vpa(2^(bits(i)-1), vpa_acc) < -1)
            left_edge = floor(vpa(2^(bits(i)-1), vpa_acc)) -1;
            right_edge = ceil(vpa(2^(bits(i)-1), vpa_acc) ) + MAX_Keys_interval;
        end

        if (right_edge - vpa(2^(bits(i)-1), vpa_acc)*2 > 1)
            left_edge = floor(vpa(2^(bits(i)-1), vpa_acc))*2 -MAX_Keys_interval;
            right_edge = ceil(vpa(2^(bits(i)-1), vpa_acc))*2 +1;
        end

        fprintf(fid,'%d,%f,%s:%s,%s\n',bits(i),alpha_to_seek(j),Dec2Hex_custom(left_edge), Dec2Hex_custom(right_edge), addresses{i});
